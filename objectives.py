...
def duplicate_row_check(customers_df):
    df = customers_df.groupBy(customers_df.columns).count().filter(col("count")>1)
    if df.count() != 0:
        print("\nDuplicates found!\n")
        return df
    else:
        print("No duplicates found!")
#duplicate_row_check(customers_df)

def duplicate_col_check(customers_df):
    l = []
    for i in customers_df.columns:
        if i in l:
            print("Duplicate col found")
            print(list(i))
        else:
...
def duplicates_in_col_check(colu):
    loc = colu
    cnt = customers_df.groupBy(loc).count().filter(col("count")>1).count()
    print(f"{cnt} duplicates found in {loc} column")
    customers_df.groupBy(loc).count().filter(col("count")>1).select(loc).distinct().show()

def col_nulls_count(customers_df):
    print("\nNull Counts : \n")
    customers_df.select([count(when(isnan(c) | col(c).isNull(), c)).alias(c) for c in customers_df.columns]).show()

def rows_with_null(customers_df):
    #P_DF = customers_df.toPandas()
    A = customers_df[customers_df.isna().any(axis=1)]
    return A
...
def main(spark):
    ...
    obj_4(customers_df)
    rows_with_null(customers_df)
    customers_df = spark.read.csv("dataset/customers.csv", inferSchema = True, header = True)
    duplicate_row_check(customers_df)
    duplicate_col_check(customers_df)
    duplicates_in_col_check("location")
    col_nulls_count(customers_df)
